...
async function encryptAndStore(tempFilePath: string): Promise<Response> {
  ...
  const lcpencryptCommand = `lcpencrypt -input "${finalFilePath.replaceAll(' ', '').trim()}" -url ${publicUrl} -contentid ${contentId} -lcpsv http://${lcp_user}:${lcp_pass}@${process.env.LCP_HOST}:8989 -verbose`;

  return new Promise((resolve) => {
    exec(lcpencryptCommand, async (error, stdout, stderr) => {
      console.log('PROBLEM', error, stdout, stderr);
